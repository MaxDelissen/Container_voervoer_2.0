...
public class ContainerRow
{
    ...
    public List<Container> FailedContainers { get; } = new();
    ...
    public bool TryAddContainer(Container container, ContainerRow? nextRow, ContainerRow? previousRow)
    {
        ...
        int leftWeight = CalculateSideWeight(ShipSide.Left);
        int rightWeight = CalculateSideWeight(ShipSide.Right);
        int centerWeight = CalculateSideWeight(ShipSide.Center);
        ...
        int totalWeight = leftWeight + rightWeight + CalculateSideWeight(ShipSide.Center);
        //if it doesn't fit on lightest side, check if it fits on other side while keeping weight difference in mind
        int difference = Math.Abs(leftWeight - rightWeight);
        decimal maxDifference = totalWeight * MaxWeightDifferencePercentage;
    ...
    private bool TryPlaceContainerOnSide(Container container, ShipSide lightestSide, ContainerRow? nextRow, ContainerRow? previousRow)
    {
        ...
        foreach (ContainerStack stack in Stacks)
        {
            ...
            {
                ContainerStack? nextStack = nextRow?.Stacks[stack.LeftRightIndex - 1];
                ContainerStack? previousStack = previousRow?.Stacks[stack.LeftRightIndex - 1];
    ...
    public ContainerRow(int shipWidth)
    {
        ...
        if (hasCenterStack)
        {
            var centerIndex = (int)Math.Ceiling(shipWidth / 2.0);
